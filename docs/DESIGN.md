...
An earlier note here claimed two older database-only posts (`Parkrun Performance Analysis` and `Climate Change Part 4`) remained published outside the canonical package set. That claim was never actually checked against production, only against a local database, where the same two posts existed as leftover pre-pipeline seed data (both dated 24 June 2026 in that copy, not 28 July 2025 as previously stated here) and were deleted locally during the `2.8.8` review. Checked against production after the `2.8.8` deploy on 22 August 2026: neither post exists there. The note is corrected rather than silently removed, since it was stated here as fact without having been verified.
...
- Confirmed on production after the `2.8.8` deploy: no legacy notebook-style posts exist outside the 13-package corpus. An earlier version of this document claimed two remained published; that claim was checked only against a local database, not production, and was wrong. As of `2.8.8`, featured selection also no longer depends on an `is_featured` flag at all: any published post can be randomly selected for either featured slot, which was itself a response to the flag requiring editorial upkeep that wasn't happening.
